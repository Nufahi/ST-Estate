...
const STRINGS = {
    en: {
        ...
        batchSizeHint: 'A place is written in several small requests instead of one big one. Fewer entries per request means more waiting and far fewer failures: a model asked for ten rooms at once tends to run out of room mid-reply and lose all of them. Raise it only if your model handles long JSON well.',
        ...
        generatingBatch: 'Batch {n} of {total}…',
        retrying: 'The reply was malformed. Asking again…',
        retryingKeys: 'The reply came back without keywords. Asking again…',
        retryingBudget: 'The model ran out of room before answering. Asking again with more…',
        ...
        toastBatchFailed: '{n} batch(es) came back unusable, so some rooms are missing. Lower "Entries per request" and run it again for those.',
    ...
    ru: {
        ...
        batchSize: 'Записей за запрос',
        batchSizeHint: 'Место пишется несколькими небольшими запросами вместо одного огромного. Чем меньше записей за раз, тем дольше ждать и тем реже всё ломается: у модели, которую попросили десять комнат сразу, ответ обрывается на середине — и пропадают все десять. Повышайте, только если ваша модель уверенно держит длинный JSON.',
        ...
        generatingBatch: 'Партия {n} из {total}…',
        retrying: 'Ответ пришёл кривой. Спрашиваю ещё раз…',
        retryingKeys: 'Ответ пришёл без ключей. Спрашиваю ещё раз…',
        retryingBudget: 'Модель израсходовала лимит ответа на размышления. Спрашиваю ещё раз, с запасом…',
        ...
        toastBatchFailed: 'Партий вернулось негодными: {n} — часть комнат не написана. Уменьшите «Записей за запрос» и прогоните ещё раз.',
